    ...
    <div>
      <div className="row pb-4" key={id}>
        ...
        <div className="col-md-7">
          ...
          <div className="short-description text-start">{excerpt(description, 120)}</div>
          <Link to={`/detail/${id}`}>
            <button className="btn btn-read">Read More</button>
          </Link>
          {user && user.uid === userId && (
            <div style={{ float: 'right' }}>
              <FontAwesome name="trash" style={{ margin: '15px', cursor: 'pointer' }} size="2x" onClick={() => handleDelete(id)} />
              <Link to={`/update/${id}`}>
